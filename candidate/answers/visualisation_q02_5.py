...
df = pd.read_csv("answer_q02_5.csv", delimiter=';', header = 0)

print(df)

df['row_number_by_group']=df.groupby(['city']).ngroup() + 1
print(df)
...
plt.barh(df.city, df.num_of_listings, color  = colors1)
#plt.barh(df.Products, df.Number_Of_Working_Days, color  = colors1)
for i, v in enumerate(df.num_of_listings):
